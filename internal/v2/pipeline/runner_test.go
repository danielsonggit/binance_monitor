...
func TestRunnerCalculateUpdatesHealth(t *testing.T) {
	...
	runner, err := NewRunner(service, 5*time.Minute, 5*time.Second, 90, slog.New(slog.NewTextHandler(io.Discard, nil)))
	if err != nil {
		t.Fatal(err)
	}
...
func TestRunnerPreservesLastSuccessWhenNextCycleFails(t *testing.T) {
	asOf := time.Date(2026, 8, 9, 12, 0, 0, 0, time.UTC)
	service := &recordingCalculateAt{result: Result{Features: feature.Result{AsOf: asOf, ValidMetrics: 4}}}
	runner, _ := NewRunner(service, 5*time.Minute, 5*time.Second, 90, slog.New(slog.NewTextHandler(io.Discard, nil)))
	if err := runner.Calculate(context.Background(), asOf); err != nil {
		t.Fatal(err)
	}
...
func TestRunnerRejectsUnalignedCycle(t *testing.T) {
	runner, _ := NewRunner(&recordingCalculateAt{}, 5*time.Minute, 5*time.Second, 90, slog.New(slog.NewTextHandler(io.Discard, nil)))
	if err := runner.Calculate(context.Background(), time.Date(2026, 8, 9, 12, 0, 1, 0, time.UTC)); err == nil {
		t.Fatal("expected alignment error")
	}
}

func TestRunnerDegradesWhenMetricCoverageIsBelowThreshold(t *testing.T) {
	asOf := time.Date(2026, 8, 9, 12, 0, 0, 0, time.UTC)
	service := &recordingCalculateAt{result: Result{
		Features: feature.Result{AsOf: asOf, ValidMetrics: 89, InvalidMetrics: 11, Written: 25},
	}}
	runner, err := NewRunner(service, 5*time.Minute, 5*time.Second, 90, slog.New(slog.NewTextHandler(io.Discard, nil)))
	if err != nil {
		t.Fatal(err)
