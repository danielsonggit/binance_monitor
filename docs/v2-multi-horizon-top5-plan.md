...
| --- | --- |
| 状态 | COMPLETED |
| 当前阶段 | MHR-7 已完成；继续保留影子服务积累 7 天生产启用样本 |
...
| MHR-7 | 影子运行、部署与验收 | COMPLETED | jmk 连续运行 24–72 小时；延迟、覆盖率、缺口达标 | 1–3 天观察期 |
...
- [x] jmk 生产配置不覆盖 V1，以独立 worker/API 服务运行，reporter 保持禁用。
- [x] 首次启动完成 universe、WebSocket、历史回补、四周期特征和 8 组排名流水线。
- [x] 连续观察超过 24 小时无时间网格断档、无 FAILED，计算延迟和有效 universe 覆盖率达标。
- [x] 人工抽样与 Binance K 线原始值复核 10 个 symbol × 4 个周期，共 40/40 通过。
- [x] 演练 V2 服务重启与专用数据库故障恢复；核验代理连接隔离，不影响 V1、7891 或其他服务。

当前观察窗口从 `2026-08-10 14:31:10 +08:00` 开始。启动前识别的 201 个历史
`MARKET_SNAPSHOT_5M` 缺口来自此前未常驻运行，作为既有审计保留，不计为影子运行后的新增断流。

前 18.5 小时共完成 221 个五分钟窗口，整体 snapshot 实际覆盖率最小 `89.106%`、
平均 `93.621%`；缺失主要来自休市或低活跃 TradFi，Crypto 最新分板块覆盖为
`548/564 = 97.163%`。四周期特征覆盖存在可重复的 15 分钟节奏：整刻约 100%、
`+5` 分钟约 96%–98%、`+10` 分钟约 61%–64%。原因是 `+10` 分钟时最近一根完成的
15m K 线距离目标恰为 10 分钟，超过既定的 5 分钟基准偏移门禁；这些指标仍保存为无效并从
榜单排除，不放宽投资数据质量规则。

因此 `SNAPSHOT_MINIMUM_RATIO_PERCENT=85`、`FEATURE_MINIMUM_COVERAGE_PERCENT=60`
只定义 worker heartbeat 的全局断流/灾难性缺数门槛。精确覆盖率、缺失数量和原因继续写入
审计/API；“HEALTHY”不代表所有标的都有有效指标。

最终观察窗口为 `2026-08-10 06:35:00Z` 至 `2026-08-11 11:55:00Z`，持续 29 小时
20 分钟，共 `353/353` 个五分钟时间点，时间网格缺口为 0。snapshot 平均覆盖
`93.234%`、最低 `88.889%`；snapshot 状态仍按“任何缺标的即 DEGRADED”的严格审计
口径保存，但均高于 85% worker 健康门槛。冷启动前四小时有 16 个特征窗口低于 60%，
原因是 24h 历史与 5m 热窗口尚未形成；`2026-08-10 10:30:00Z` 后未再低于健康门槛。

生产启用门槛没有因此缩短：本文专项 MHR-7 的 24 小时验收已完成，但正式 V2 Telegram
通知仍遵循产品文档的至少 7 天影子观察，并且需要用户单独授权启用 reporter。
...
### 2026-08-10 — MHR-7 影子部署启动

- V2 PostgreSQL 继续使用 `binance-radar-v2_postgres-v2-data` Docker 卷，只向宿主机
  `127.0.0.1:54329` 发布；端口变更重建容器后 schema 6、716 个 active instrument、
  2,864 条特征和 16 个历史榜单头均保持不变。
- 新增 `binance-radar-v2-worker.service` 与 `binance-radar-v2-api.service` user units；均启用
  自动失败重启、资源上限、只读系统保护和 journald 日志，user linger 已启用。
- worker/API 使用静态 `binance-monitor-v2`；运行器仅允许 `worker|api` 两个角色，强制 Binance
  出网为 `127.0.0.1:7890`。未安装 reporter unit。
- V1 `binance-monitor.service` 在部署前后均为 active；V2 worker 的实际 TCP 连接只指向
  `127.0.0.1:7890` 与 `127.0.0.1:54329`，没有连接 7891。
- V2 API 只监听 `127.0.0.1:18080`，live/ready 均通过。
- 首次分析 `as_of=2026-08-10T06:30:00Z`：716 个标的、2,860/2,864 个有效指标、
  8 个排名组、40 个榜单项、历史回补缺口 0；worker/API 均为 0 次重启。
- MHR-7 仍为 `IN_PROGRESS`，必须完成 24–72 小时连续观察、40 组人工复核和故障演练后才能关闭。

### 2026-08-11 — MHR-7 前 18.5 小时校准

- 221 个连续五分钟窗口均完成 snapshot、四周期计算与排名流水线；worker、API、V1 的
  `NRestarts` 均为 0，未启动 reporter。
- snapshot 整体覆盖率最小 `89.106%`、平均 `93.621%`；最新 Crypto 覆盖
  `548/564 = 97.163%`，低活跃 TradFi 是主要缺失来源。
- 特征有效率呈确定的 15m 完成 K 线节奏：整刻约 100%、`+5` 分钟约 96%–98%、
  `+10` 分钟约 61%–64%；无效原因主要为基准价偏移超过 5 分钟，而非计算错误。
- 增加 snapshot 与特征覆盖率 heartbeat 门禁。健康法定人数分别设为 85% 和 60%；原始缺失
  仍完整审计，无效收益仍不得进入榜单，计算质量门禁没有放宽。

### 2026-08-11 — MHR-7 完成

- 完成 29 小时 20 分钟连续观察：三个流水线各有 353 个五分钟窗口、0 个 FAILED；snapshot
  时间网格为 `353/353`，不存在未执行窗口，平均覆盖 93.234%、最低 88.889%。
- 发现 4 个当天新上市 TradFi 合约导致 453 个虚假历史缺口；接入 Binance `onboardDate`，
  回补分母从每个合约实际上市 K 线开始计算。部署后 backfill 为 `SUCCEEDED / missing=0`。
- 对 BTC、ETH、SOL、XRP、DOGE、黄金、白银、原油、MU、SNDK 的 15m/1h/4h/24h
  进行 Binance 原始 15m K 线独立计算，40/40 通过，最大误差小于 `5e-13` 个百分点。
- V2 专用 PostgreSQL 停止期间 API readiness 正确返回 503；恢复后约 1 秒返回 200，
  数据行数保持不变，worker/API 无失败重启，V1 全程 active。
- V2 worker 实际 TCP 连接仅指向 `127.0.0.1:7890` 和 `127.0.0.1:54329`，没有 7891
  连接。7890 同时承载 V1，因此没有人为停止共享代理；代理中断行为由客户端单元/race 测试覆盖，
  线上验收采用连接隔离与持续 29 小时外部访问证据，避免为了演练破坏 V1。
- V1、V2 worker、V2 API 的 `NRestarts` 均为 0；V2 reporter unit 不存在，outbox 为 0，
  没有发送 Telegram。数据库约 482 MiB，jmk 剩余约 1.5 TiB。
- 全仓 `go test ./...`、`go vet ./...`、受影响模块 race、Compose 配置、systemd 脚本语法和
  `git diff --check` 均通过。MHR-7 标记为 `COMPLETED`，服务继续影子运行积累 7 天样本。
