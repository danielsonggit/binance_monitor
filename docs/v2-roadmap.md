...
| 最后更新 | 2026-08-30 |
| 当前主阶段 | R3 已完成；R4-A1 schema 8 已部署，正在进行 24–48 小时影子验收 |
| 下一开发阶段 | 完成 R4-A1 容量、滞回和换手验收，再决定是否进入 R4-B 分层采集 |
...
| R4 | 候选机会池与分层采集 | IN_PROGRESS | R4-A1 schema 8 已部署，24–48 小时影子验收进行中 |
...
1. R3 的 24 小时运行验收和 operational coverage 修正已经完成；
2. R4-A0 七天只读分布分析已经完成，冷启动流动性门槛已固化为 `candidate-rules-v1`；
3. R4-A1 已于 2026-08-30 部署 schema 8，首个完整窗口写入 872 条评估和 14 个 ACTIVE；
4. 连续观察 24–48 小时的容量截断、换手、三窗口退出、冷却、失败和资源使用；
5. 影子验收完成前不进入 R4-B，不启用 V2 Telegram reporter，也不把候选解释为买入信号。
